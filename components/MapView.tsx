...
import * as maplibregl from "maplibre-gl"; // Unused in this Leaflet implementation but kept for compatibility
...
const icons = {
  residential: createCategoryIcon(
    "#2563eb",
  ...
  convenience: createCategoryIcon(
    "#ea580c",
  ...
  public_service: createCategoryIcon(
    "#475569",
...
const GeodesicCircle: React.FC<{
  ...
  return (
    <Polygon positions={points} pathOptions={pathOptions} interactive={false} />
  );
};

const DistanceMarkers: React.FC<{ center: Location; radius: number }> = ({
  center,
  radius,
}) => {
  ...
  return (
    <>
      ...
      />
      <Marker
        position={[north.lat, north.lng]}
        icon={createLabelIcon(labelText)}
        interactive={false}
      />
      <Marker
        position={[east.lat, east.lng]}
        icon={createLabelIcon(labelText)}
        interactive={false}
      />
      <Marker
        position={[south.lat, south.lng]}
        icon={createLabelIcon(labelText)}
        interactive={false}
      />
      <Marker
        position={[west.lat, west.lng]}
        icon={createLabelIcon(labelText)}
        interactive={false}
      />
    </>
...
const PlaceMarker: React.FC<{ item: PlaceItem; icon: L.DivIcon }> = ({
  item,
  icon,
}) => {
  const map = useMap();
    ...
    <Marker position={[item.lat!, item.lng!]} icon={icon}>
      <Popup>
        <div className="font-sans min-w-[200px]">
          <strong className="text-sm block mb-1 text-slate-800">
            {item.name}
          </strong>
          {item.rating && (
...
const AnalyzedPlaceMarkers: React.FC<{
  ...
  const allMarkers = useMemo(() => {
    ...
    const add = (
      ...
      // Filter: if selectedCategory is provided and not 'all', skip if key doesn't match
      if (
        selectedCategory &&
    ...
    add(analysis.residential, icons.residential, "residential");
    add(analysis.convenience, icons.convenience, "convenience");
    add(analysis.shopping, icons.shopping, "shopping");
    add(analysis.food, icons.food, "food");
    add(analysis.transport, icons.transport, "transport");
    add(analysis.recreation, icons.recreation, "recreation");
    add(analysis.public_service, icons.public_service, "public_service");
      ...
      if (item.lat && item.lng) {
        ...
        const stars = item.rating
          ...
          <div class="flex items-center gap-1 mb-2">
            <span class="text-amber-500 font-bold text-sm">★ ${item.rating.toFixed(
              1
            )}</span>
        ...
        const marker = L_Global.marker([item.lat, item.lng], {
          icon: icon,
        }).bindPopup(container);
...
const HeatmapLayer: React.FC<{
  ...
  useEffect(() => {
    ...
    const extract = (items: PlaceItem[], categoryKey: Category) => {
      // Filter logic
      if (
        selectedCategory &&
        selectedCategory !== "all" &&
    ...
    extract(analysis.residential, "residential");
    extract(analysis.convenience, "convenience");
    extract(analysis.shopping, "shopping");
    extract(analysis.food, "food");
    extract(analysis.transport, "transport");
    extract(analysis.recreation, "recreation");
    extract(analysis.public_service, "public_service");
...
const MapView: React.FC<MapViewProps> = ({
  ...
  return (
    <div className="h-full w-full z-0 relative">
        ...
        {(viewMode === "markers" || viewMode === "heatmap") && (
          <>
            ...
            <Marker position={[center.lat, center.lng]} icon={DefaultIcon}>
              <Popup>
                <div className="text-center">
                  <strong className="text-indigo-600">จุดกึ่งกลาง</strong>
      ...
      {/* Attribution Badge */}
      <div className="absolute bottom-1 right-1 bg-white/80 px-2 py-0.5 text-[10px] text-slate-500 rounded z-[400]">
        MapRange AI
      </div>
    </div>
  );
